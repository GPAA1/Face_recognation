...
def add_data_face(password, name):
    cap = cv2.VideoCapture(0)
    count = 0
    key = 'faceR'

    if len(password) != len(key):
        print('Неверный пароль, доступ в базу данных закрыт')
        return False
    else:
        for char1, char2 in zip(password, key):
            if char1 != char2:
                print('Неверный пароль, доступ в базу данных закрыт')
                return False
        print('Смотрите в камеру, вас внесут в базу данных')
        while True:
            ret, frame = cap.read()
...
def add_face():
    password = simpledialog.askstring("Input", "Введите пароль для доступа в базу данных", show='*')
    if password is None:
        return
    name = simpledialog.askstring("Input", "Введите ваше имя")
    if name is None:
        return

    if add_data_face(password, name):
...
recognize_button = tk.Button(root, text="Распознать лицо", command=recognize_face)
recognize_button.pack(pady=10)

add_face_button = tk.Button(root, text="Добавить биометрию в базу данных", command=add_face)
add_face_button.pack(pady=10)

encrypt_data_button = tk.Button(root, text="Зашифровать данные", command=encrypt_data)
